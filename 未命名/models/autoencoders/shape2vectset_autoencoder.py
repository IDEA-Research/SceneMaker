...
@craftsman.register("shape2vectset-autoencoder")
class Shape2VectSetAutoencoder(AutoEncoder):
    ...
    def encode(self,
        ...
        if self.cfg.use_fps:
            target_samples = max(1, min(self.cfg.num_latents, N))
            sampled_pc_list = []
            for b in range(B):
                pc_np = pc[b, :, :3].detach().float().cpu().numpy()
                kdline_fps_samples_idx = fpsample.bucket_fps_kdline_sampling(pc_np, target_samples, h=5)
                sampled_pc_list.append(pc[b, kdline_fps_samples_idx])
            sampled_pc = torch.stack(sampled_pc_list, dim=0)
        else:
            sampled_pc = pc
