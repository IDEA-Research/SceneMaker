...
@craftsman.register("t5-encoder")
class T5Encoder(BaseCaptionEncoder):
    ...
    def configure(self) -> None:
        ...
        if self.cfg.pretrained_t5_name_or_path is not None:
            ...
            if self.cfg.pretrained_model_name_or_path is None: # default to load t5-base model
                assert self.cfg.t5_type is not None, "The t5_type should be provided"
                print(f"Loading T5 model from {self.cfg.t5_type}")
                self.text_model = T5EncoderModel(config=T5EncoderModel.config_class.from_pretrained(
                            self.cfg.t5_type,
                ...
                self.text_model = T5EncoderModel.from_pretrained(self.cfg.t5_type, torch_dtype=torch.bfloat16)
                ...
                raise ValueError(f"Unknown T5 model: {self.cfg.pretrained_model_name_or_path}")
