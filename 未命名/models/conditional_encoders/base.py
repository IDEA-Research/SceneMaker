...
class BaseEmbedder(BaseModule):
    ...
    def encode_text(self, texts: Iterable[str], **kwargs) -> torch.FloatTensor:
        pass
    ...
    def forward_visual_embeds(self, batch):
        assert "image" in batch or "mvimages" in batch, "image or mvimages is required for visual embeds"
    ...
    def forward_text_embeds(self, batch):
        assert "caption" in batch, "caption is required for text embeds"
        ...
        if self.cfg.normalize_embeds: # post-process the text embeds
