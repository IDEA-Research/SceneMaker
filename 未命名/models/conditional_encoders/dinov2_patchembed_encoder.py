...
@craftsman.register("dinov2-patchembed-embedder")
class Dinov2Embedder(BaseEmbedder):
    ...
    def configure(self) -> None:
        ...
        # add patch embed part
        patch_size = self.dino_model.config.patch_size
        hidden_size = self.dino_model.config.hidden_size
        self.patch_embed = nn.Sequential(
            nn.Conv2d(3, 4 * hidden_size, kernel_size=(patch_size, patch_size), stride=patch_size),
            nn.Conv2d(4 * hidden_size, hidden_size, kernel_size=(1, 1)),
        )
        nn.init.zeros_(self.patch_embed[1].weight)
        nn.init.zeros_(self.patch_embed[1].bias)
    ...
    def encode_image_dino(self, images: Iterable[Optional[ImageType]], cameras: Optional[torch.Tensor] = None, force_none_camera_embeds: bool = False, return_dict: bool = False, **kwargs) -> torch.FloatTensor:
        ...
        pixel_values = rearrange(pixel_values.to(self.dino_model.device), "B N C H W -> (B N) C H W")
        if self.cfg.encode_camera and camera_embeds is not None:
            vision_outputs = self.dino_model(pixel_values, condition=rearrange(camera_embeds, "B N C -> (B N) C"))
        else:
        ...
        if return_dict:
            ...
            return pixel_values.to(vision_outputs.last_hidden_state), dino_embeds_dict
        else:
            return pixel_values.to(vision_outputs.last_hidden_state), vision_outputs.last_hidden_state
    ...
    def encode_image(self, images: Iterable[Optional[ImageType]], cameras: Optional[torch.Tensor] = None, force_none_camera_embeds: bool = False, return_dict: bool = False, **kwargs) -> torch.FloatTensor:
        pixel_values, dino_embeds = self.encode_image_dino(images, cameras)
        ...
        patch_embeds = patch_embeds.view(patch_embeds.shape[0], self.dino_model.config.hidden_size, -1).permute(0, 2, 1)

        fused_embeds = torch.cat([dino_embeds[:, :1], dino_embeds[:, 1:] + patch_embeds], dim = 1)
