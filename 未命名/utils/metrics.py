...
# ):
#     """
#     Compute Chamfer Distance between predicted and ground truth point clouds using PyTorch3D.
...
#     loss, loss_norm = chamfer_distance(pred, gt, batch_reduction=None)
    
#     # dist1: distances from pred to gt
#     # dist2: distances from gt to pred
#     # Both have shape (B,)
    
#     return loss, loss/2, loss/2


import kaolin as kal
def compute_chamfer_distance(pred, gt, reduction='mean'):
    """
    使用 Kaolin 计算 Chamfer Distance
    """
    distance = kal.metrics.pointcloud.chamfer_distance(pred, gt)
    
    # kaolin 直接返回平均的 chamfer distance
    total_loss = distance
    cd1 = distance / 2  # 近似分解
    cd2 = distance / 2
    
    return total_loss, cd1, cd2
    
    # # 应用 reduction
    # if reduction == 'mean':
    #     return torch.mean(cd_loss)
    # elif reduction == 'sum':
    #     return torch.sum(cd_loss)
    # elif reduction == 'none':
    #     return cd_loss
    # else:
