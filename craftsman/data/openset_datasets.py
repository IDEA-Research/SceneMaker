...
from utils.space_transform import get_rot_from_pitch, get_pitch_from_R, q2rot, get_yaw_from_R, get_rot_from_yaw, convert_camera_coordinate
...
@dataclass
class OpensetDataModuleConfig(BaseS3DataModuleConfig):    
    ################################# Scene #################################
    scene_list: str = "/comp_robot/shiyukai/datasets/openset_scene/debug/data/all_json_files.json"
    geo_data_path: str = "/comp_robot/shiyukai/datasets/objaverse/objaverse_clean/high_quality_xl_190k_images/geometry/sample/"
    # deocc_image_path: str = '/comp_robot/shiyukai/datasets/instPiFU/datasets/render_obj_scene_img/'
    caption_path: str = "/comp_robot/shiyukai/datasets/objaverse/captions/captions/Cap3D_automated_Objaverse_full.csv"
...
class OpensetDataset(Dataset):
    def __init__(self, config, mode):
        super(OpensetDataset, self).__init__()
        self.mode=mode
        self.cfg: OpensetDataModuleConfig = config
        assert mode in ["train", "val", "test"]

        # import scene json files
        with open(self.cfg.scene_list, 'r') as f:
            self.all_scenes = json.load(f)["json_files"]
        ...
        if self.mode == "train" and self.cfg.train_indices is not None:
            ...
            # self.splits = (self.cfg.test_indices[0]+30, self.cfg.test_indices[0]+40)
        else:
            self.splits = (0, len(self.all_scenes))

        self.all_scenes = self.all_scenes[self.splits[0] : self.splits[1]]
        
        self.caption_list = pd.read_csv(self.cfg.caption_path, header=None)
    ...
    def _load_shape_from_occupancy_or_sdf(self, surface_path, scale=1) -> Dict[str, Any]:
        ...
        if self.cfg.geo_data_type == "occupancy":
            ...
            if self.cfg.with_sharp_data:
                sharp_surface = data["surface"].copy()
        else:
    ...
    def __getitem_scene(self, index):
        ...
        scene_data_path = scene.replace('.json', '')
        scene_id = scene_data_path.split('/')[-1]
        with open(scene, 'r') as f:
            scene_info = json.load(f)
        with open(os.path.join(scene_data_path, "metadata.json"), 'r') as f:
            cam_info = json.load(f)
    
        # camera pose
        cam_id = random.randint(0, self.cfg.num_views-1) if self.mode == "train" else index%3
        camera_K, cam2wrd = np.array(cam_info["intrinsics"][cam_id]), np.array(cam_info["cam2world_matrixs"][cam_id])
        # blender to opengl coord
        cam2wrd = convert_camera_coordinate(cam2wrd, target="opengl", original="blender")
        if self.cfg.only_use_pitch:
            cam2wrd[:3,:3] = get_rot_from_yaw(get_yaw_from_R(cam2wrd[:3, :3]))
        
        # load image
        image_path = os.path.join(scene_data_path, f"{cam_id:04d}_rgb.png") # augmentation data
        image = np.array(Image.open(image_path).convert("RGB"))
        height, width = image.shape[0], image.shape[1]
        # Image.fromarray(image).save("outputs/viz/scene_img.png")

        # depth
        depth_path = os.path.join(scene_data_path, f"{cam_id:04d}_depth.png")
        depth = cv2.imread(depth_path, cv2.IMREAD_ANYCOLOR|cv2.IMREAD_ANYDEPTH).astype(np.float32)
        depth = depth / depth.max() * 20.0  # convert to meter
        # rescale depth
        ...
        n_objects = len(scene_info['objects'])
        ...
        miss_obj = False # whether miss object
        for object_ind in range(n_objects):
            if object_ind >= self.cfg.max_objs:
                break
            
            # load furniture info
            object_info = scene_info['objects'][object_ind]
            jid = object_info['uuid']
            
            # load transformation opengl
            rotation_mat = np.array(object_info['rotation_matrix']).reshape(4, 4)
            translation = np.array(object_info['translation']).reshape(3, 1)
            tran_matrix = np.eye(4)
            tran_matrix[:3, :3] = rotation_mat[:3, :3]
            tran_matrix[:3, 3:] = translation
            ...
            bbox_size = np.ones((3,)) * object_info['scale'] # from half size to full size
            bbox_sizes.append(bbox_size*np.ones_like(bbox_size))

            # load object shape
            jid_filename = os.path.basename(jid)  # 获取文件名
            jid_dir = os.path.basename(os.path.dirname(jid)) 
            surface_path = os.path.join(self.cfg.geo_data_path, jid_dir, jid_filename.replace(".glb", ".npz"))
            size = bbox_size / bbox_size.max()
            try:
                ret = self._load_shape_from_occupancy_or_sdf(surface_path=surface_path, scale=size)
                geo_legal = 1
            except Exception as e:
                # print(f"Warning: Failed to load shape from {surface_path}: {e}")
                # 返回全0的值，保持形状相同
                ret = {
                    "surface": np.zeros((self.cfg.n_samples, 6), dtype=np.float32),  # [n_samples, 6] for points+normals
                }
                if self.cfg.with_sharp_data:
                    ret["sharp_surface"] = np.zeros((16384, 6), dtype=np.float32)  # [16384, 6] for sharp surface
                miss_obj = True
                geo_legal = 0
            
            # augmentation on geo shape
            ...
            # object caption
            caption = self.caption_list[self.caption_list[0] == jid_filename.replace(".glb","")][1].values[0]
            class_list.append(caption) # category
            
            # mask
            instance_mask_path = os.path.join(scene_data_path, f"{cam_id:04d}_seg.png")
            ...
            instance_mask = (instance_mask == (object_ind + 2)).astype(np.uint8) * 255  # 1 is plane
            ...
            if "full_pcd" in self.cfg.translation_mode:
                pass
            elif (obj_pcd_cam.shape[0] <= self.cfg.min_pcd) or geo_legal == 0:
                obj_pcd_cam = torch.zeros(self.cfg.min_pcd, 3)
                pcd_tran = np.zeros(3)
                pcd_size = 1e-4
            ...
            mask_legal[object_ind] = geo_legal * mask_legal[object_ind] # if no geometry, then illegal
        ...
        if self.cfg.translation_mode == "pcd" or "pcd_aug" in self.cfg.translation_mode:
            ...
            pcd_sizes = np.stack(pcd_sizes, axis=0)
            if mask_index.shape[0] > 0:
                # modify active object pcd to layout space
        ...
        data_dict = {"whole_img": image, "pose": rotation, "translation": translation, "size": bbox_sizes[...,:1], "trans_matrix": tran_matrix_cam, "surface": geo_shape, "sharp_surface": geo_shape_sharp, \
                "image": crop_imgs, "obj_pcds": obj_pcds, "scene_pcds": scene_pcds, "pcd_sizes": pcd_sizes[...,:1], "pcd_trans": pcd_trans, "mask_legal": mask_legal, "taskid": scene_id, "masks": masks, \
...
# data loader  
@register("Openset-datamodule")
class OpensetDataModule(pl.LightningDataModule):
    cfg: OpensetDataModuleConfig

    def __init__(self, cfg: Optional[Union[dict, DictConfig]] = None) -> None:
        super().__init__()
        self.cfg = parse_structured(OpensetDataModuleConfig, cfg)

    def setup(self, stage=None) -> None:
        if stage in [None, "fit", "train"]:
            self.train_dataset = OpensetDataset(self.cfg, "train")
        if stage in [None, "fit", "validate"]:
            self.val_dataset = OpensetDataset(self.cfg, "test")
            self.train_dataset = OpensetDataset(self.cfg, "train")
        if stage in [None, "test", "predict"]:
            self.test_dataset = OpensetDataset(self.cfg, "test")
...
if __name__ == "__main__":
    ...
    import argparse
    parser = argparse.ArgumentParser(description="Quick debug for OpensetDataset/DataModule")
    ...
    if args.cfg is not None:
        ...
        cfg = OpensetDataModuleConfig(**cfg_dict)
    else:
        cfg = OpensetDataModuleConfig()

    # 初始化数据集
    dataset = OpensetDataset(cfg, args.mode)
