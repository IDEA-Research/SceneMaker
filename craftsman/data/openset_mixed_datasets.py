...
from .openset_datasets import OpensetDataModuleConfig, OpensetDataset

import pdb

class OpensetMixedDataset(Dataset):
    def __init__(self, config, mode):
        super(OpensetMixedDataset, self).__init__()
        self.mode = mode
        ...
        openset_cfg_dict = dict(config.get("openset_cfg", {}))
        public_cfg = {k: v for k, v in config.items() if k not in ["instpifu_cfg", "midi_cfg", "openset_cfg"]}
        
        # construct config
        instpifu_cfg_dict = {**instpifu_cfg_dict, **public_cfg}
        midi_cfg_dict = {**midi_cfg_dict, **public_cfg}
        openset_cfg_dict = {**openset_cfg_dict, **public_cfg}
        self.instpifu_cfg = Front3DReconDataModuleConfig(**instpifu_cfg_dict)
        self.midi_cfg = Front3DMIDIDataModuleConfig(**midi_cfg_dict)
        self.openset_cfg = OpensetDataModuleConfig(**openset_cfg_dict)
        ...
        self.openset_datasets = OpensetDataset(self.openset_cfg, mode)
        

    def __len__(self):
        return len(self.instpifu_datasets) + len(self.midi_datasets) + len(self.openset_datasets)

    def __getitem__(self, index):
        if index < len(self.instpifu_datasets):
            return self.instpifu_datasets[index]
        elif index < len(self.instpifu_datasets) + len(self.midi_datasets):
            return self.midi_datasets[index - len(self.instpifu_datasets)]
        else:
            return self.openset_datasets[index - len(self.instpifu_datasets) - len(self.midi_datasets)]
...
# data loader  
@register("Openset-mixed-datamodule")
class OpensetMixedDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage=None) -> None:
        if stage in [None, "fit"]:
            self.train_dataset = OpensetMixedDataset(self.cfg, "train")
        if stage in [None, "fit", "validate"]:
            self.val_dataset = OpensetMixedDataset(self.cfg, "test")
            self.train_dataset = OpensetMixedDataset(self.cfg, "train")
        if stage in [None, "test", "predict"]:
            self.test_dataset = OpensetMixedDataset(self.cfg, "test")
