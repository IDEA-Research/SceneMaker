...
class CLIP_Dinov2_Embedder(BaseEmbedder):
    ...
    def configure(self) -> None:
        ...
        if not self.cfg.encode_camera:
            ...
            if self.cfg.pretrained_dino_name_or_path is not None:
                ...
                if self.cfg.pretrained_model_name_or_path is None: # default to load Dinov2 model
                    ...
                    self.dino_model: Dinov2WithRegistersModel = Dinov2WithRegistersModel(config=ConditionalDinov2Model.config_class.from_pretrained(
                        "facebook/dinov2-with-registers-large",
                    ))
                elif "dinov2reg" in self.cfg.pretrained_model_name_or_path:
                ...
                elif "dinov2base" in self.cfg.pretrained_model_name_or_path or "clip-dino-" in self.cfg.pretrained_model_name_or_path: # for leagcy
                    print("Loading Dinov2 model from facebook/dinov2-base")
                    self.dino_type = "facebook/dinov2-base"
                    self.dino_model: Dinov2Model = Dinov2Model(config=ConditionalDinov2Model.config_class.from_pretrained(
                        "facebook/dinov2-base",
        ...
        if self.cfg.zero_uncond_embeds:
            image_size = max(self.cfg.image_size_clip, self.cfg.image_size_dino)
            self.empty_image_embeds_clip = torch.zeros((self.cfg.n_views, (image_size // 14) ** 2 + 1, 1024)).detach()
            self.empty_image_embeds_dino = torch.zeros((self.cfg.n_views, (image_size // 14) ** 2 + 1, 1024)).detach()
            ...
            if self.cfg.encode_camera:
                ...
                self.empty_image_embeds_dino = self.encode_image_dino(torch.zeros(self.cfg.n_views, self.cfg.image_size_dino, self.cfg.image_size_dino, 3), self.cameras[:self.cfg.n_views]).detach()
            else:
            ...
            self.empty_image_embeds_clip, self.empty_image_embeds_dino = self.align_clip_dino(self.empty_image_embeds_clip, self.empty_image_embeds_dino)
            self.empty_image_embeds = torch.cat([self.empty_image_embeds_clip, self.empty_image_embeds_dino], dim=1)
        ...
        if self.clip_model.config.vision_config.hidden_size != self.dino_model.config.hidden_size:
            self.linear_proj = nn.Linear(self.dino_model.config.hidden_size, self.clip_model.config.vision_config.hidden_size, bias=False)
        else:
    ...
    def encode_image_clip(self, images: Iterable[Optional[ImageType]], cameras: Optional[torch.Tensor] = None, force_none_camera_embeds: bool = False, return_dict: bool = False, **kwargs) -> torch.FloatTensor:
        ...
        if isinstance(images, (np.ndarray, torch.Tensor)): # for training process
            ...
            if self.cfg.encode_camera:
                ...
                camera_embeds = self.encode_camera(cameras)
            pixel_values = self.image_preprocess_clip.preprocess(images, return_tensors='pt', \
                    do_rescale=True, do_resize=True, size=self.cfg.image_size_clip, crop_size=self.cfg.image_size_clip).pixel_values
    ...
    def align_clip_dino(self, clip_embeds, dino_embeds):
        if clip_embeds.shape[-2] != dino_embeds.shape[-2]: # different shape, interpolate the clip embeddings to the same shape as dino embeddings
            assert clip_embeds.shape[-2] == (self.cfg.image_size_clip // 14) ** 2 + 1, "The clip embeddings should have the shape of (n_views, (image_size_clip // 14) ** 2 + 1, 1024)"
            clip_embeds_patch_tokens = clip_embeds[:, 1:].view(clip_embeds.shape[0], self.cfg.image_size_clip // 14, self.cfg.image_size_clip // 14, 1024)
            clip_embeds_patch_tokens = torch.nn.functional.interpolate(clip_embeds_patch_tokens.permute(0, 3, 1, 2), \
                        size=(self.cfg.image_size_dino // 14, self.cfg.image_size_dino // 14), mode='bilinear', align_corners=False).permute(0, 2, 3, 1).view(clip_embeds.shape[0], -1, 1024)
            clip_embeds = torch.cat([clip_embeds[:, :1], clip_embeds_patch_tokens], dim=1)
